...
func (tx *Tx) init() {
	tx.ActiveInfo = NewActiveInfoClient(tx.config)
	tx.Algorithm = NewAlgorithmClient(tx.config)
	tx.AlgorithmAlarmType = NewAlgorithmAlarmTypeClient(tx.config)
	tx.Area = NewAreaClient(tx.config)
	tx.Camera = NewCameraClient(tx.config)
	tx.Device = NewDeviceClient(tx.config)
	tx.DeviceAlgo = NewDeviceAlgoClient(tx.config)
	tx.DeviceCamera = NewDeviceCameraClient(tx.config)
	tx.DeviceToken = NewDeviceTokenClient(tx.config)
	tx.EquipAttr = NewEquipAttrClient(tx.config)
	tx.EventSubscription = NewEventSubscriptionClient(tx.config)
	tx.FileUpload = NewFileUploadClient(tx.config)
	tx.Inform = NewInformClient(tx.config)
	tx.Organization = NewOrganizationClient(tx.config)
	tx.Setting = NewSettingClient(tx.config)
	tx.Signature = NewSignatureClient(tx.config)
	tx.Task = NewTaskClient(tx.config)
	tx.TaskCamera = NewTaskCameraClient(tx.config)
	tx.TaskLimits = NewTaskLimitsClient(tx.config)
	tx.UpPlatform = NewUpPlatformClient(tx.config)
	tx.User = NewUserClient(tx.config)
	tx.WarnPush = NewWarnPushClient(tx.config)
	tx.WarnPushLog = NewWarnPushLogClient(tx.config)
	tx.WarnSmsErr = NewWarnSmsErrClient(tx.config)
	tx.WarningType = NewWarningTypeClient(tx.config)
}
...
// applies a query, for example: ActiveInfo.QueryXXX(), the query will be executed
