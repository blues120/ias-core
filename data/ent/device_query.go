...
func (dq *DeviceQuery) QueryCamera() *CameraQuery {
	query := (&CameraClient{config: dq.config}).Query()
...
// QueryTaskDevice chains the current query on the "task_device" edge.
func (dq *DeviceQuery) QueryTaskDevice() *TaskQuery {
...
func (dq *DeviceQuery) QueryDeviceCamera() *DeviceCameraQuery {
	query := (&DeviceCameraClient{config: dq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (dq *DeviceQuery) Clone() *DeviceQuery {
	...
	return &DeviceQuery{
		...
		withCamera:       dq.withCamera.Clone(),
		withTaskDevice:   dq.withTaskDevice.Clone(),
		withDeviceCamera: dq.withDeviceCamera.Clone(),
...
func (dq *DeviceQuery) WithCamera(opts ...func(*CameraQuery)) *DeviceQuery {
	query := (&CameraClient{config: dq.config}).Query()
...
// WithTaskDevice tells the query-builder to eager-load the nodes that are connected to
// the "task_device" edge. The optional arguments are used to configure the query builder of the edge.
func (dq *DeviceQuery) WithTaskDevice(opts ...func(*TaskQuery)) *DeviceQuery {
	query := (&TaskClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withTaskDevice = query
	return dq
}
...
func (dq *DeviceQuery) WithDeviceCamera(opts ...func(*DeviceCameraQuery)) *DeviceQuery {
	query := (&DeviceCameraClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withDeviceCamera = query
	return dq
}
...
func (dq *DeviceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Device, error) {
	var (
		...
		loadedTypes = [3]bool{
			dq.withCamera != nil,
			dq.withTaskDevice != nil,
			dq.withDeviceCamera != nil,
		}
	)
	...
	if query := dq.withCamera; query != nil {
		if err := dq.loadCamera(ctx, query, nodes,
			func(n *Device) { n.Edges.Camera = []*Camera{} },
			func(n *Device, e *Camera) { n.Edges.Camera = append(n.Edges.Camera, e) }); err != nil {
			return nil, err
		}
	}
	if query := dq.withTaskDevice; query != nil {
		if err := dq.loadTaskDevice(ctx, query, nodes,
			func(n *Device) { n.Edges.TaskDevice = []*Task{} },
			func(n *Device, e *Task) { n.Edges.TaskDevice = append(n.Edges.TaskDevice, e) }); err != nil {
			return nil, err
		}
	}
	if query := dq.withDeviceCamera; query != nil {
		if err := dq.loadDeviceCamera(ctx, query, nodes,
			func(n *Device) { n.Edges.DeviceCamera = []*DeviceCamera{} },
			func(n *Device, e *DeviceCamera) { n.Edges.DeviceCamera = append(n.Edges.DeviceCamera, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DeviceQuery) loadCamera(ctx context.Context, query *CameraQuery, nodes []*Device, init func(*Device), assign func(*Device, *Camera)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(device.CameraTable)
		s.Join(joinT).On(s.C(camera.FieldID), joinT.C(device.CameraPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(device.CameraPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(device.CameraPrimaryKey[0]))
...
func (dq *DeviceQuery) loadTaskDevice(ctx context.Context, query *TaskQuery, nodes []*Device, init func(*Device), assign func(*Device, *Task)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(task.FieldDeviceID)
	}
	query.Where(predicate.Task(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(device.TaskDeviceColumn), fks...))
	}))
...
func (dq *DeviceQuery) loadDeviceCamera(ctx context.Context, query *DeviceCameraQuery, nodes []*Device, init func(*Device), assign func(*Device, *DeviceCamera)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Device)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(devicecamera.FieldDeviceID)
	}
	query.Where(predicate.DeviceCamera(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(device.DeviceCameraColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (dq *DeviceQuery) ForUpdate(opts ...sql.LockOption) *DeviceQuery {
...
func (dq *DeviceQuery) ForShare(opts ...sql.LockOption) *DeviceQuery {
	if dq.driver.Dialect() == dialect.Postgres {
		dq.Unique(false)
	}
	dq.modifiers = append(dq.modifiers, func(s *sql.Selector) {
